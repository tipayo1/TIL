...
# - 노드 시그니처: (state, config[RunnableConfig]) 패턴
...
# 정책(분기 판단) – Studio에서 확인 가능
from policy import need_expand, decide_after_xp  # noqa: F401

# ---------------- Env / constants ----------------
_MAX_EXPANDS = int(os.getenv("RPG_MAX_EXPANDS", "3"))
_MAX_K = int(os.getenv("RPG_MAX_K", "24"))
_MAX_FLAT_ROUNDS = int(os.getenv("RPG_MAX_FLAT_ROUNDS", "2"))
...
def _compute_metrics(query: str, docs: List[Document], must_terms: List[str], filters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "negative_rate": neg,
        "novel_evidence_contrib": 0.0,  # award_xp에서 갱신
...
def compose_rpg(state: State, config: Optional[RunnableConfig] = None) -> State:
    ...
    hints: Dict[str, Any] = {
        ...
        "rerank": "overlap" if not os.getenv("RERANK_CE_ENABLE", "").strip() else "cross_encoder",
        "params": {"k": 8},
    }
    ...
    # 루프 제어 카운터 초기화
    state["expand_count"] = int(state.get("expand_count", 0))
    state["flat_rounds"] = int(state.get("flat_rounds", 0))
    state["k_zero_rounds"] = int(state.get("k_zero_rounds", 0))
    state["last_coverage"] = float(state.get("last_coverage", 0.0))
    state["stagnant_rounds"] = int(state.get("stagnant_rounds", 0))

    # thread_id를 로그에 남김(Studio 가시화)
    tid = ((config or {}).get("configurable") or {}).get("thread_id") if config else None  # type: ignore
    if tid:
...
def retrieve_rpg(state: State, config: Optional[RunnableConfig] = None) -> State:
    ...
    pairs: List[Tuple[Document, float]] = vs.similarity_search_with_score(  # type: ignore
        query, k=k, filters=filters, must_terms=must_terms, fetch_k=fetch_k
    )
    docs = [p[0] for p in (pairs or [])] if pairs else []

    # 0건 라운드 카운터
    state["k_zero_rounds"] = int(state.get("k_zero_rounds", 0)) + 1 if len(docs) == 0 else 0
...
def award_xp(state: State, config: Optional[RunnableConfig] = None) -> State:
    ...
    - xp/xp_total 갱신
    - 평탄 라운드(flat_rounds) 및 정체(stagnant_rounds) 관리
    """
    ...
    # flat_rounds: 새로운 문서가 전혀 없으면 +1, 있으면 0으로 리셋
    state["flat_rounds"] = int(state.get("flat_rounds", 0)) + 1 if novel_rate == 0.0 else 0

    # coverage 정체 추적
    metrics = dict(state.get("retrieval_metrics", {}) or {})
    cur_cov = float(metrics.get("coverage", 0.0))
    last_cov = float(state.get("last_coverage", 0.0))
    delta = abs(cur_cov - last_cov)
    state["stagnant_rounds"] = int(state.get("stagnant_rounds", 0)) + 1 if delta < 1e-3 else 0
    state["last_coverage"] = cur_cov
...
def expand_search(state: State, config: Optional[RunnableConfig] = None) -> State:
    ...
    # k 증가 + 상한
    cur_k = int(params.get("k", 8))
    new_k = min(cur_k + 4, _MAX_K)
    params["k"] = new_k
    params["diversity_boost"] = True
    params["fetch_k"] = max(int(params.get("fetch_k", 0)), new_k * 3)

    hints["params"] = params
    state["retrieval_hints"] = hints

    # 확장 횟수 추적
    state["expand_count"] = int(state.get("expand_count", 0)) + 1

    # 메타 플로우 로그
    meta = dict(state.get("rpg", {}) or {})
    flows = list(meta.get("flows", []))
    flows.append({"at": "expand_search", "k": new_k})
    meta["flows"] = flows
    state["rpg"] = meta
    return state
...
def generate_answer(state: State, config: Optional[RunnableConfig] = None) -> State:
    ...
    hm = HumanMessage(
        content=f"{get_prompt(templ, 'generate_answer')}\n\nPlan:\n{state.get('answer_plan')}\n\nEvidence:\n{joined}\n\nQuery:\n{state.get('refined_query') or state.get('query','')}"
    )
    out = llm.invoke([sys, hm])
