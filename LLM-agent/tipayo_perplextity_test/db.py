...
# 확장자 확장: 코드/설정 포맷 포함
_ALLOWED_EXTS = [
    ".txt", ".md", ".pdf", ".docx", ".html", ".htm", ".pptx",
    ".py", ".ipynb", ".rst", ".json", ".yaml", ".yml", ".toml",
]
...
def _load_yaml(path: str) -> List[Document]:
    try:
        ...
        return [Document(
            page_content=dumps(obj, ensure_ascii=False, indent=2),
            metadata={"source": path}
        )]
    except Exception:
        return _load_txt(path)
    ...
    for p in paths:
        for fp in _iter_paths(p):
            ...
            if ext == ".txt":
                ...
                got = _load_yaml(fp)
            elif ext in [".toml", ".rst", ".ipynb"]:
                got = _load_txt(fp)  # 최소 폴백
            else:
                continue
            for d in got:
...
class _VSAdapter:
    ...
    def similarity_search_with_score(self, query: str, k: int, namespace=None, meta_filter=None):
        return self._inner.similarity_search_with_score(query, k, filter=meta_filter, namespace=namespace)
