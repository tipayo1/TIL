# rpg.py (minimal but complete + validation rules)
...
from policy import need_expand, EPSILON
...
class DataFlowManager:
    ...
    def validate(self, frm: str, to: str, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        """
        Minimal flow checks to accumulate actionable violations.
        """
        violations: List[str] = []
        docs = snapshot.get("documents") or []
        metrics = snapshot.get("retrieval_metrics") or {}

        if frm == "retrieve_rpg" and to == "award_xp":
            if not docs:
                violations.append("documents must not be empty after retrieval")
        if to == "plan_answer":
            if not metrics:
                violations.append("retrieval_metrics must exist before planning")
        if to == "generate_answer":
            if not snapshot.get("answer_plan"):
                violations.append("answer_plan must exist before generation")
...
def configure_rag_for_context(context_type: str, question: str) -> RPGGraph:
    ...
    g.add_root(retr); g.add_root(rer); g.add_root(gen)
    return g
